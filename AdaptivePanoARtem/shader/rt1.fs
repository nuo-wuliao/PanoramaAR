			...
			if (traceGbuffer(ray_orig, ray_dir, normal, li_rad, hitnormal, hitpos, hitmat2, hitpixel)) {
				if (hitmat2 == 0) {
					...
					if(luminance(pp) < luminance(envtraceRes)){//
						//pp = pp.zyz;
						tempirradRV += min(1.0f, pow(1 * length(hitpos - ray_orig), shadowfade))* pp;//min(1.0f, pow(length(hitpos - ray_orig), shadowfade)) * pp;//length(hitpos - ray_orig) * pp;//7.0 * min(1.0f, shadowfade * length(hitpos - ray_orig))
...
//         vec3 irrad = vec3(1e-4), irradRV = vec3(1e-4);
//         vec3 right = (abs(normal.y) == 1.f)?vec3(1,0,0):normalize(cross(vec3(0,1,0), normal));
//         vec3 up = normalize(cross(normal, right));
...
//         vec3 envtraceRes;
//         fastSSRTrace(ray_orig, ray_dir, normal, li_rad);
//         envtraceRes = li_rad*costheta / pdf;
//         irrad += envtraceRes;
...
//                 //next tracing
// 				//irradRV = vec3(0);
//                 irradRV += iterateScatter(ray_dir, hitpos, hitnormal, hitmat2, nextRand(seed), nextRand(seed)) *costheta / pdf;
//             }
//         }
//         else
//         {
...
// 		if(ratio.x !=ratio.x || ratio.y !=ratio.y || ratio.z !=ratio.z) ratio = vec3(1.f);
//         rtNewColor = vec4(texture(colorTex, TexCoords).rgb * ratio, 1);
//     }
// }