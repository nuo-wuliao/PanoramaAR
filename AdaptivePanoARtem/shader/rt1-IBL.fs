			...
			if(rayZMax > sceneDepth)
			{
				if(lod <= 0){
					if(currenthitmat == 0){
						...
						if(lasthitmat == currenthitmat || abs(rayZMax - sceneDepth)<0.1){
							hitflag = true;
							//to do
...
vec3 iterateScatter(vec3 dir, vec3 hitpos, vec3 hitnormal, float hitmat, float u, float v)
{
	vec3 result = vec3(0);
	if (hitmat == 1) {
		float costheta, pdf;
...
void main()
{
    ...
    if(hitmat == 0){
        ...
        //         irradRV = envtraceRes;
		// 		tempirradRV = irradRV;
		// 		traceDataRV = vec3(1,0,0);
        //     }
        //     else {
        //         //next tracing
		// 		//irradRV = vec3(0);
        //         vec3 pp = iterateScatter(ray_dir, hitpos, hitnormal, hitmat2, nextRand(seed), nextRand(seed)) *PI;//costheta / pdf;
		// 		tempirradRV = min(1.0f, pow(length(hitpos - ray_orig), shadowfade)) * pp;//7.0 * min(1.0f, shadowfade * length(hitpos - ray_orig))
		// 		irradRV = pp;// 10.0 * length(hitpos - ray_orig) * pp;
		// 		traceDataRV = vec3(1,1,0);
        //     }
        // }
        // else
        // {
