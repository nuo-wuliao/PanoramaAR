		...
		//if(hitTexCoords.x >= 1 || hitTexCoords.x < 0 || hitTexCoords.y >= 1 || hitTexCoords.y < 0 || testP.x * stepDir > end) {
			...
			if(rayZMax > sceneDepth) // && currenthitmat == 0
			{
				if(lod <= 0){
					...
					rayZMax - sceneDepth < 0.01 || abs(rayZMax - sceneDepth)<0.001)
					{
						return 0;
					}
					//return 0;
					...
					// }
					// else {//
					if((lastSceneDepth - sceneDepth > 0.01 && abs(rayZMax - sceneDepth) > 10.f * length(dQ / testk))// 
						&& abs(dot(hitnormal, normalize(last_hitpos - hitpos)))>0.01 && lasthitmat == currenthitmat){//
...
bool sampleBRDF2(float u, float v, vec3 world_wo, vec3 right, vec3 up, vec3 n, vec3 pos, inout float pdf, inout vec3 orig, 
    inout vec3 dir, inout vec3 li_rad, inout vec3 hitnormal, inout vec3 hitpos, inout int hitmat, inout vec2 hitpixel, bool isFirstBounce, inout int state)
{
	li_rad = vec3(0.f);

	if(dot(n, world_wo) < 0) {
		vec3 temp = world_wo - 2 * dot(world_wo, n) * n;
		world_wo = temp;
	}
	...
	else
	{
		fastSSRTrace(orig, dir, n, li_rad);
		li_rad = computeLightRadiance(vec3(0, 0, 1), wo, wi, li_rad) / pdf;
		//li_rad = vec3(dot(n, world_wo));
		//li_rad = vec3(1,0,0);
		//li_rad = texture(panoTex, vec2(li_rad.y / (2*PI), li_rad.x / PI)).rgb;
		state = 0;
	}
	return true;
}

vec3 iterateScatter(vec3 dir, vec3 hitpos, vec3 hitnormal, int hitmat, float u, float v, vec3 rho)
{
	vec3 result = vec3(0);
	if (hitmat == 1) {
		float costheta, pdf;
	...
	if (hitmat == 2) {
		...
		if (sampleBRDF2(u, v, -dir, right, up, hitnormal, hitpos, pdf, nex_orig, nex_dir, li_rad, nex_hitnormal, nex_hitpos, 
            nex_hitmat,nex_hitpixel, false, state))
		{
			result = li_rad;
		}
		//else result = vec3(1,1,0);
	}
...
void main()
{
	vec2 jittorTexCoords = TexCoords + vec2(xOff,yOff)/iResolution;
    float hitmat = texture(mat_prevCoordTex, jittorTexCoords).x;
    if(hitmat!=0){
        vec3 irradRV = vec3(0), li_rad = vec3(0);
        vec2 iPos = jittorTexCoords * (iResolution - vec2(1,1));
        int offset = int(iPos.x + iPos.y * iResolution.x);
        vec3 pixelpos = texture(worldPosTex, jittorTexCoords).rgb;
        vec3 normal = texture(normalTex, jittorTexCoords).rgb;
		...
		for(int i = 0;i<SPP;i++){
			...
			if(hitmat == 1){
				...
				if(state!=2){
					if(state == 0){
						if(hitmat2 == 0){
							vec4 temp = texture(newColorTex, vec2(hitpixel / iResolution));
							li_rad = temp.rgb * temp.w;
							irradRV += li_rad * PI;// costheta /pdf;
						}
						else{
							vec2 hitpixelTex = vec2(hitpixel/iResolution);
							vec3 rho = texture(diffTextureTex, hitpixelTex).rgb;
							vec3 temp = iterateScatter(ray_dir, hitpos, hitnormal, hitmat2, nextRand(seed), nextRand(seed),rho);
							irradRV += temp * PI;// costheta /pdf;
							traceDataRV = temp;//vec3(1,1,1);
						}
					}
					else{
			...
			else if(hitmat == 2)
			{
				...
				if(sampleBRDF2(uv.x, uv.y, -ray_direction, right, up, normal, pixelpos, pdf, ray_orig, ray_dir, li_rad, 
					hitnormal, hitpos, hitmat2, hitpixel, true, _state)) {
					if(hitmat2 == 0){
						irradRV += li_rad;
						traceDataRV = vec3(0,0,0);
					}
						...
						if(hitmat2 == 1) {
							vec2 hitpixelTex = vec2(hitpixel/iResolution);
							rho = texture(diffTextureTex, hitpixelTex).rgb;
							temp = rho * 0.3;
						}
						else{
							temp = iterateScatter(ray_dir, hitpos, hitnormal, hitmat2, nextRand(seed), nextRand(seed), rho);
						}
						irradRV += temp;
						traceDataRV = vec3(1,0,0);//temp;//vec3(1, 1, 1);
					}
				}
			}
		}
